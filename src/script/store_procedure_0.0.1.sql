...
UPDATE MEMBER
SET PRE_CRL=CASE WHEN ISNULL(M.PRE_CRL,'')='' OR M.PRE_CRL<P.CRL THEN P.CRL ELSE M.PRE_CRL END,
    PRE_SCORE=CASE WHEN ISNULL(M.PRE_SCORE,'')='' OR M.PRE_SCORE<T.PRE_INDEX THEN T.PRE_INDEX ELSE M.PRE_SCORE END,
    PRE_RATING=CASE WHEN (ISNULL(M.PRE_SCORE,'')='' OR M.PRE_SCORE<T.PRE_INDEX) AND PRE_INDEX>1.2 THEN 'A'
                    WHEN M.PRE_SCORE>=T.PRE_INDEX AND M.PRE_SCORE>1.2 THEN 'A'
                    WHEN (ISNULL(M.PRE_SCORE,'')='' OR M.PRE_SCORE<T.PRE_INDEX) AND PRE_INDEX BETWEEN 1 AND 1.2 THEN 'B'
                    WHEN M.PRE_SCORE>=T.PRE_INDEX AND M.PRE_SCORE BETWEEN 1 AND 1.2 THEN 'B'
                    WHEN (ISNULL(M.PRE_SCORE,'')='' OR M.PRE_SCORE<T.PRE_INDEX) AND PRE_INDEX BETWEEN 0.5 AND 1 THEN 'C'
                    WHEN M.PRE_SCORE>=T.PRE_INDEX AND M.PRE_SCORE BETWEEN 0.5 AND 1 THEN 'C'
                    WHEN (ISNULL(M.PRE_SCORE,'')='' OR M.PRE_SCORE<T.PRE_INDEX) AND PRE_INDEX<0.5 THEN 'D'
                    WHEN M.PRE_SCORE>=T.PRE_INDEX AND M.PRE_SCORE<0.5 THEN 'D'
               ELSE 'D' END
